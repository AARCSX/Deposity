...
export default function CreateVehicleWizard({ isOpen, onClose, onSubmit, vehicleToEdit }: CreateVehicleWizardProps) {
  ...
  const handleAddStatePermit = () => {
    ...
    if (permitData.states.some(s => s.name === selectedState)) return; // No duplicates
    setPermitData(prev => ({
      ...prev,
      states: [...prev.states, { name: selectedState, issuance: "", expiry: "" }],
    }));
    setSelectedState("");
  };
  ...
  const handleStatePermitChange = (stateName: string, field: "issuance" | "expiry", value: string) => {
    setPermitData(prev => ({
      ...prev,
      states: prev.states.map(s => s.name === stateName ? { ...s, [field]: value } : s),
    }));
  };
  ...
  const availableStates = INDIAN_STATES.filter(s => !permitData.states.some(ps => ps.name === s));
              ...
              <div className="border-t border-outline-variant/15 pt-6">
                ...
                <div className="mb-5">
                  <Select 
                    ...
                    onChange={(v) => setPermitData(prev => ({ ...prev, type: v as "National" | "State" }))} 
                    options={["National", "State"]} 
                  />
                </div>

                {/* National Permit — Simple issuance + expiry */}
                {permitData.type === "National" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 p-4 bg-surface-container-low rounded-xl border border-outline-variant/10">
                    ...
                    <Input 
                      ...
                      type="date" 
                      value={permitData.issuance} 
                      onChange={(v) => setPermitData(prev => ({ ...prev, issuance: v }))} 
                    />
                    <Input 
                      label="Permit Expiry Date" 
                      type="date" 
                      value={permitData.expiry} 
                      onChange={(v) => setPermitData(prev => ({ ...prev, expiry: v }))} 
                    />
                  </div>
                )}

                {/* State Permit — Add per-state entries */}
                {permitData.type === "State" && (
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3 items-end">
                      ...
                      <button
                        ...
                        className="px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold disabled:opacity-40 hover:opacity-90 transition-opacity flex items-center gap-1 shrink-0"
                      >
                    ...
                    {/* State Permits List */}
                    {permitData.states.length === 0 ? (
                      <div className="py-8 text-center border border-dashed border-outline-variant/20 rounded-xl bg-surface-container-low">
                      ...
                      <div className="space-y-3">
                        {permitData.states.map((sp) => (
                          <div key={sp.name} className="p-4 bg-surface-container-low rounded-xl border border-outline-variant/10">
