...
func (h *Handler) Create(c *gin.Context) {
	...
	activity.LogActivity(h.db, activity.LogActivityParams{
		...
		EntityType:  "vehicle",
		EntityID:    item.ID,
		Description: fmt.Sprintf("%s registered new vehicle %s (%s)", middleware.GetUserName(c), item.Core.RegistrationNumber, item.Core.Make),
...
func (h *Handler) Update(c *gin.Context) {
	...
	activity.LogActivity(h.db, activity.LogActivityParams{
		...
		EntityType:  "vehicle",
		EntityID:    item.ID,
		Description: fmt.Sprintf("%s updated details for vehicle %s", middleware.GetUserName(c), item.Core.RegistrationNumber),
		IPAddress:   c.ClientIP(),
	})
...
func (h *Handler) Delete(c *gin.Context) {
	...
	activity.LogActivity(h.db, activity.LogActivityParams{
		...
		EntityType:  "vehicle",
		EntityID:    id,
		Description: fmt.Sprintf("%s deleted vehicle record #%s", middleware.GetUserName(c), id),
		IPAddress:   c.ClientIP(),
	})
